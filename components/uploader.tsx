    ...
    <section className="rounded-[28px] border border-white/70 bg-white/90 p-4 shadow-soft backdrop-blur sm:p-6">
      {!file ? (
        ...
        >
          ...
          <h2 className="mt-5 font-display text-2xl font-semibold text-ink">{t("dropPdf")}</h2>
          ...
          <p className="mt-4 text-xs text-slate-400">{t("uploadLimit")}</p>
          <input
            ref={inputRef}
            type="file"
        ...
        <div>
          <div className="flex items-center gap-4 rounded-2xl border border-moss-100 bg-moss-50 p-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-xs text-slate-500">{formatBytes(file.size)} · {t("ready")}</p>
            </div>
            {!loading && (
              <button
                type="button"
          ...
          >
            {loading ? (
              <><LoaderCircle className="h-5 w-5 animate-spin" /> {t("analyzing")}</>
            ) : (
              <><FileCheck2 className="h-5 w-5" /> {t("analyze")} <ArrowRight className="h-4 w-4" /></>
            )}
          </button>
        </div>
      )}
